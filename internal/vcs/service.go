...
func (s *VCSService) SendVoiceTransmission(channelID string) error {
	...
	s.LogEvent("ED-137", "TX", "INFO", fmt.Sprintf("[%s] 🗣️ ATC Controller Voice TX: \"Tokyo Tower, AeroVoice 123. Radio check, radio check. 1 2 3 4 5.\"", ch.cfg.Name))

	// Automatically release PTT after speech ends (~10.44s duration + 560ms buffer)
	go func() {
